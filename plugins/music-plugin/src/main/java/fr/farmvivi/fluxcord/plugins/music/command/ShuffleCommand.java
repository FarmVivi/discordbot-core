...
public class ShuffleCommand {
    ...
    public ShuffleCommand(MusicPlugin plugin, AudioPlayerService audioPlayerService) {
        this.plugin = plugin;
    ...
    public CommandResult execute(CommandContext context) {
        ...
        if (musicPlayer.getQueueSize() == 0) {
            context.reply("❌ The queue is empty! Add some songs first.");
            return CommandResult.error("Empty queue");
        }
        
        if (musicPlayer.getQueueSize() == 1) {
            context.reply("❌ Need at least 2 songs in the queue to shuffle!");
            return CommandResult.error("Not enough songs");
        }
        
        // Shuffle the queue
        musicPlayer.shuffleQueue();
        
        context.reply("🔀 Queue shuffled! (" + musicPlayer.getQueueSize() + " songs randomized)");
